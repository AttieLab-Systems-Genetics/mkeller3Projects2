...
data <- data |>
  mutate(GenLit = as.factor(GenLit),
         Sex = as.factor(Sex),
         Diet = as.factor(Diet),
         Diet.name = as.factor(Diet.name),
...
with(AUCdata, table(GenLit, Batch))

anova(lm(D_Urea_AUC ~ Batch, data = data))
anova(lm(D_Urea_AUC ~ Sex*Diet+Batch, data = data))

ggplot(data, aes(x = Batch, y = D_Urea_AUC, color = Diet.name)) +
...
ggplot(dataAdj, aes(x = Batch, y = D_Urea_AUC.Adj, color = GenLit)) +
  geom_point(position = position_jitter(width = 0.1, height = 0)) +
  theme(axis.text.x = element_text(angle=90, hjust=1, vjust=0.5))


ggplot(dataAdj, aes(x = Batch, y = D_Urea_AUC.Adj, color = Diet.name, shape = Sex)) +
  geom_point(position = position_jitter(width = 0.1, height = 0), size = 3) +
  theme(axis.text.x = element_text(angle=90, hjust=1, vjust=0.5))


ggplot(combined, aes(x = Batch, y = D_Urea_AUC, color = Diet.name, shape = Sex)) +
  geom_point(position = position_jitter(width = 0.1, height = 0), size = 3) +
  theme(axis.text.x = element_text(angle=90, hjust=1, vjust=0.5))

anova(lm(D_Urea_AUC ~ Sex * Diet + Batch, data = combined))
anova(lm(D_Urea_AUC.Adj ~ Sex * Diet + Batch, data = combined))


Adj <- ggplot(combined, aes(x = Diet.name, y = D_Urea_AUC.Adj, color = Diet.name)) +
  geom_boxplot(outlier.shape = NA) +
  geom_point(position = position_jitter(width=0.2, height=0)) +
  facet_wrap(~Sex) + 
  theme_bw(base_size = 18)

NonAdj <- ggplot(combined, aes(x = Diet.name, y = D_Urea_AUC, color = Diet.name)) +
  geom_boxplot(outlier.shape = NA) + 
  geom_point(position = position_jitter(width=0.2, height=0)) +
  facet_wrap(~Sex) +
  theme_bw(base_size = 18)
...
write.csv(combined, "D_AUCs_Adj.csv")

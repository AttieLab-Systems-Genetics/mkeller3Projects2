...
#===============================================================================
# 1) Extract module gene SYMBOLS into a "hotspot-like" table
...
enrich_modules_fast <- function(module_tables,
  ...
  ann <- as.data.table(AnnotationDbi::select(org.Mm.eg.db, keys = keys_entrez,
                                             columns = c("GO","ONTOLOGY"), keytype = "ENTREZID"))
  ann <- ann[!is.na(GO) & ONTOLOGY == ont, .(GO, ENTREZID)]
  TERM2GENE <- unique(data.frame(term = ann$GO, gene = ann$ENTREZID, stringsAsFactors = FALSE))
  
  t2n <- AnnotationDbi::select(GO.db, keys = unique(ann$GO), columns = "TERM", keytype = "GOID")
  TERM2NAME <- unique(data.frame(term = t2n$GOID, name = t2n$TERM, stringsAsFactors = FALSE))
  
  # Map all symbols -> ENTREZ once across ALL modules
  all_syms <- unique(unlist(lapply(mod_list, \(dt) unlist(dt$symbols, use.names = FALSE)),
                            use.names = FALSE))
  all_syms <- unique(all_syms[!is.na(all_syms) & nzchar(all_syms)])
  sym_map <- suppressMessages(bitr(all_syms, fromType = "SYMBOL", toType = "ENTREZID",
  ...
  filter_trim <- function(dt) {
    if (!nrow(dt)) return(dt)
    dt[, go := lapply(go, function(ego) {
      if (is.null(ego) || is.null(ego@result) || !nrow(ego@result)) return(NULL)
      res <- ego@result
      keep <- (!is.na(res$Count)) & res$Count >= 5 & (!is.na(res$qvalue)) & res$qvalue <= 0.05
      res2 <- res[keep, , drop = FALSE]
      if (!nrow(res2)) return(NULL)
      ego@result <- res2
      ego
    })]
    dt[sapply(go, \(x) !is.null(x) && !is.null(x@result) && nrow(x@result) > 0)]
  }
  ...
  flatten <- function(dt) {
    ...
    rows <- lapply(seq_len(nrow(dt)), function(i) {
      ego <- dt$go[[i]]; if (is.null(ego)) return(NULL)
  ...
  diag_tbl <- function(name, dt) {
    if (is.null(dt) || !nrow(dt)) return(data.table(dataset=name, n_modules=0, med_syms=NA_real_, med_entrez=NA_real_))
    ns <- lengths(dt$symbols)
    ne <- vapply(dt$symbols, \(s) length(map_syms(s)), integer(1))
    data.table(dataset=name, n_modules=nrow(dt),
  ...
  results <- lapply(names(mod_list), function(nm) {
    dt <- as.data.table(mod_list[[nm]])
    if (!nrow(dt)) {
      return(list(name = nm, enr = data.table(), diag = diag_tbl(nm, dt), terms = data.table()))
    }
    uni <- universe_for(dt)
    entrez_list <- lapply(dt$symbols, map_syms)
    idx <- which(vapply(entrez_list, length, 0L) >= min_set)
    
    go_list <- vector("list", nrow(dt))
    if (length(idx)) {
      go_list[idx] <- future_lapply(idx, function(i) {
        gene <- entrez_list[[i]]
        ego <- tryCatch(
          enricher(gene = gene, universe = uni,
                   TERM2GENE = TERM2GENE, TERM2NAME = TERM2NAME,
                   pAdjustMethod = "BH",
                   pvalueCutoff = p_cut, qvalueCutoff = q_cut,
                   minGSSize = minGSSize, maxGSSize = maxGSSize),
          error = function(e) NULL
        )
        if (!is.null(ego) && readable) {
          ego <- tryCatch(setReadable(ego, OrgDb = org.Mm.eg.db, keyType = "ENTREZID"),
    ...
    dt[, `:=`(go = go_list, n_entrez = vapply(entrez_list, length, 0L))]
    dt_f <- filter_trim(dt)
    
    list(
      name = nm,
      enr = dt,                        # table with go objects per module
      diag = diag_tbl(nm, dt),
      terms = flatten(dt_f)              # flat term table (one row per enriched term)
    )
  })
  
  list(
    diagnostics = rbindlist(lapply(results, `[[`, "diag"), fill = TRUE),
    enrichment_results = setNames(lapply(results, `[[`, "enr"), lapply(results, `[[`, "name")),
    enriched_terms = setNames(lapply(results, `[[`, "terms"), lapply(results, `[[`, "name"))
  )
}
...
# Write out data
fwrite(mod_enrichment$enriched_terms$modules, file = "./modules_enrichment.csv", sep = ",")
